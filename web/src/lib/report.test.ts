...
describe("companion report", () => {
  ...
  it("mails support@x.ai only for API bugs", () => {
    ...
    assert.ok(mailtoHref(api).length <= 1800);
  });

  it("keeps mailto under browser URL limits when the report is long", () => {
    const api = filled({
      product: "api",
      extra: "x".repeat(8000),
      userAgent: "Mozilla/5.0 ".repeat(40),
    });
    const href = mailtoHref(api);
    assert.match(href, /^mailto:support@x\.ai\?/);
    assert.ok(href.length <= 1800);
    assert.match(href, /truncated/);
  });
