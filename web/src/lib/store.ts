...
const STORAGE_KEY = "xai-bug-reporter:v2";
const MAX_DRAFTS = 16;
    ...
    set((state) => {
      ...
      return {
        draft: {
          ...
          platform: detectPlatformFromUa(detected.userAgent),
        },
      };
    }),
  saveDraft: () => {
    ...
    set((state) => {
      ...
      return {
        draft: {
          ...state.draft,
          ...detected,
          platform: state.draft.platform ?? detectPlatformFromUa(detected.userAgent),
        },
      };
    }),
  hydrate: () => {
    ...
    const detected = detectSystemInfo();
    const inferredPlatform = detectPlatformFromUa(detected.userAgent);
    if (persisted) {
      const draft = systemFilled(persisted.draft)
        ? persisted.draft
        : { ...persisted.draft, ...detected };
      if (!draft.platform) draft.platform = inferredPlatform;
